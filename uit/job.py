...
class PbsJob:
    ...
    @property
    def job_number(self):
        return re.split(r"\.|\[", self.job_id)[0]  # noqa: W605
    ...
    def set_default_post_processing_script(self, execution_block):
        self.post_processing_script = PbsScript(
            ...
            project_id=self.script.project_id,
            num_nodes=1,
    ...
    def resolve_path(self, path):
        ...
        if self.post_processing_job_id:
            path = path.replace(
                "$POST_PROCESSING_JOB_NUMBER",
                self._clean_job_id(self.post_processing_job_id),
            )
    ...
    async def update_statuses(cls, jobs, as_df=False):
        ...
        for job in jobs:
            ...
            if (
                status["elapsed_time"] == "--"
                and status["status"] not in ("Q", "H", "B")  # noqa: W503
                and not isinstance(job, PbsArrayJob)  # noqa: W503
            ):
    ...
    def instance(cls, script, job_id, working_dir, client=None, status=None):
        instance = cls(script=script, client=client, working_dir=working_dir)
